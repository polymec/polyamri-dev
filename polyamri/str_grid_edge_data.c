...
static void count_edges(str_grid_edge_data_t* edge_data)
{
  ...
  while (str_grid_edge_data_next_x_patch(edge_data, &pos, &ip, &jp, &kp, &patch))
  {
  ...
  pos = 0;
  while (str_grid_edge_data_next_y_patch(edge_data, &pos, &ip, &jp, &kp, &patch))
  {
  ...
  pos = 0;
  while (str_grid_edge_data_next_z_patch(edge_data, &pos, &ip, &jp, &kp, &patch))
  {
    int nx = patch->i2 - patch->i1 + 1;
    int ny = patch->j2 - patch->j1 + 1;
